...
**Adjacent sub-shapes.** Several swallowed-exception sub-shapes captured in the marianne-ai-compose#308 specimen (predicate-returning-bare-False, validator-returning-False-on-error, fire-and-forget exception discard) are real but documented under this umbrella rather than as separate entries. If a future audit produces enough cross-context evidence on the predicate-returning-False shape specifically (where the function's name promises a truthy/falsy answer but the falsy case secretly means "broke"), that could be promoted to its own entry. For now, all sub-shapes are documented here.
