...
Three captured specimens, each from a different AI-coded Python codebase, each illustrating a distinct sub-shape. Detailed specimen notes are not included in the public repository.

- **[agno-agi/agno#7405](https://github.com/agno-agi/agno/issues/7405)** — canonical `open()` without `with`/`close()` at three sites. AI agent framework with CLAUDE.md (7145 bytes). Sites: `tools/openai.py:90` (audio transcription), `utils/pickle.py:15` (pickle write), `utils/pickle.py:27` (pickle read). The pickle sites use the one-liner-with-side-effect shape (`pickle.dump(obj, file_path.open("wb"))`). Long-running-agent failure mode named explicitly.
- **[HomericIntelligence/ProjectScylla#1877](https://github.com/HomericIntelligence/ProjectScylla/issues/1877)** — streaming-with-noqa-justification, discipline-risk. Three file handles in `executor/capture.py:118-122` annotated `# noqa: SIM115`. CLAUDE.md (21286 bytes). The audit framework is "repo-analyze-strict v3.0.0 (ProjectHephaestus skill)" — multi-section weighted scoring with anti-inflation grading.
- **[emerzon/litellm#56](https://github.com/emerzon/litellm/issues/56)** — producer-consumer-cleanup-split contract. `get_audio_file_for_health_check()` returns an open file handle that callers never close. CLAUDE.md (14619 bytes); structured "BUG_CONFIRMED-..." validated-report audit workflow with explicit reproduction (`/proc/self/fd` count grows 4→9 after 5 calls).
...
- *Files passed to an API that documents it will close them.* A few APIs take ownership of a file object's lifetime and close it for you — but most do not, so verify the documentation rather than assuming. (`tarfile.open(fileobj=f)`, for instance, does *not* close `f`: the Python docs state "fileobj is not closed when TarFile is closed", so the caller still owns it.) The cue is whether the API explicitly documents ownership transfer; absent that, the handle is still yours to close.
