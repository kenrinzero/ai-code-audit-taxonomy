...
- **Single-site drift in same-project-knows-right-pattern** — one file uses f-string while the rest of the codebase consistently uses `%`-style. Captured in knowitcz/gen-ai-ops-04-30#48 via the **"Hledac → Oponent → Soudce"** Czech-language multi-stage AI audit pipeline.
...
The fix is mechanical at scale: a dedicated tool such as [`lazy-log-formatter`](https://github.com/dmar1n/lazy-log-formatter) — a pre-commit hook that rewrites f-string log calls into lazy `%`-style formatting — or a libcst codemod can convert the dominant pattern automatically. Manual review is needed for multi-arg or expression-containing f-strings (`logger.info(f"User {user.name} ({user.id}) failed login attempt #{user.fail_count + 1}")` doesn't translate trivially). The IBM specimen explicitly recommends this hybrid approach: codemod for the dominant shape, manual review for the residue, ruff G004 for regression coverage.

## Notes

**Category `observability`.** Both this entry and [`print-instead-of-logging`](print-instead-of-logging.md) stem from the model producing string-output primitives that ignore the deployment-context cost. Together they form a tight observability-defect cluster within the AI-pedagogical-bias family.
...
**Connection to [`ai-pedagogical-bias`](../notes/ai-pedagogical-bias.md) note.** This entry is one of six members of the AI-pedagogical-bias meta-family, joining [`narrating-comments`](narrating-comments.md), [`print-instead-of-logging`](print-instead-of-logging.md), [`hardcoded-config-values`](hardcoded-config-values.md), [`missing-network-timeout`](missing-network-timeout.md), and [`assert-for-runtime-validation`](assert-for-runtime-validation.md). The pattern of "modern Python tutorial style produces correct-looking but production-suboptimal output" now spans six surfaces (comments, output primitives, configuration, networking, log-message formatting, and runtime validation). The cross-cutting note's promotion criterion (5+ entries) is met.
...
**Connection to [`codified-guidance-is-insufficient`](../notes/codified-guidance-is-insufficient.md) note.** IBM AGENTS.md explicitly states "Lazy logging style preferred"; the codebase has ~1000+ violations. This is now a 16+ entry observation. The IBM specimen is the highest-volume instance of codified-guidance-insufficient captured to date.

**AI-on-AI defect discovery as an emerging audit-framework class.** The Hledac/Oponent/Soudce three-stage AI pipeline (knowitcz/gen-ai-ops-04-30), combined with bot-authored audits (Cogtrix) and AI-on-AI review (jparson2389/aetherflow's Copilot+Codex catching off-by-one), illustrates that AI-driven audit infrastructure is itself an emergent pattern.
