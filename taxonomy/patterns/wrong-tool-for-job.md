...
1. **Brace escape collision.** `str.format()` uses `{...}` for substitution. HTML uses `{...}` for inline styles, CSS variables, JavaScript template literals, and `style="..."` attributes. The first time someone adds `style="--accent: {{ user.color }};"` to the template — or even uses CSS custom properties like `style="grid-template-columns: {layout_columns} auto;"` — the format call breaks because `str.format` tries to interpret the inner braces as substitutions. The bug manifests as `KeyError: '--accent: '` or as garbled output.
...
Three captured specimens, each in an AI-related project. Specimens live in `evidence/github-issues/`.

- **[safishamsi/graphify#197](https://github.com/safishamsi/graphify/issues/197)** — wrong-abstraction-layer. A Claude Code skill packages every pipeline step as inline `python -c "..."` shell commands; the wrong layer triggers Claude Code's security heuristics (~30 user prompts per run). The project already exposes CLI subcommands (`graphify query`, `graphify hook`, etc.) that would be the right tool. Specimen: [safishamsi-graphify-197.md](../../evidence/github-issues/2026-05-15-safishamsi-graphify-197.md).
- **[NousResearch/hermes-agent#15636](https://github.com/NousResearch/hermes-agent/issues/15636)** — wrong-deployment-pattern. macOS launchd plist hardcodes `ProgramArguments[0] = $venv/bin/python -m hermes_cli.main`; macOS uses `basename(ProgramArguments[0])` as Login Item display name, so the entry shows as "python" rather than "hermes." *The same project uses the correct pattern (named entry-point binary `${HERMES_VENV}/bin/hindsight-embed`) at another site* — asymmetric outlier. Confirmed Claude Sonnet 4.6 co-authorship. Fourth AI-typical pattern captured from this codebase. Specimen: [NousResearch-hermes-agent-15636.md](../../evidence/github-issues/2026-05-15-NousResearch-hermes-agent-15636.md).
- **[mem0ai/mem0#5006](https://github.com/mem0ai/mem0/issues/5006)** — wrong-API-option-within-library. Valkey vector store schema uses `TAG` (exact-match filter) instead of `TEXT` (full-text search) for the `memory` field. Users can't do partial-text search over stored memories. The justifying comment ("for Valkey compatibility") does not survive verification — Valkey has supported `TEXT` since initial release. Specimen: [mem0ai-mem0-5006.md](../../evidence/github-issues/2026-05-15-mem0ai-mem0-5006.md).
...
- **`str.format()` or f-strings building HTML strings.** If the rendered content is HTML and the project has a template engine (Flask + Jinja2, FastAPI + Jinja2, Django templates, any framework that ships templates), check whether the right tool is being used. Brace collisions and XSS surface are the defect paths.
...
- **Justifying comments that don't survive verification.** `# Using TAG instead of TEXT for Valkey compatibility` when Valkey has long supported TEXT. The comment narrates a defensive choice that doesn't reflect a real constraint — and is itself a signal that the choice was reflexive rather than informed.
...
**Connection to [`defensive-choice-with-justifying-comment`](../notes/defensive-choice-with-justifying-comment.md) note.** The mem0 specimen's `# Using TAG instead of TEXT for Valkey compatibility` is doing the same work as the [`swallowed-exceptions`](swallowed-exceptions.md) hermes-agent specimen's `# Memory is optional`: a comment that *justifies* a defensive choice in terms of a constraint that does not actually exist. The reader sees the comment and trusts it; the audit step (verify the constraint) is rarely performed. This entry is one of 7+ in the cross-cutting note.

**Connection to [`codified-guidance-is-insufficient`](../notes/codified-guidance-is-insufficient.md) note.** The aabtzu#48 audit lists `str.format()` for HTML as an AI-tell; aabtzu's own project hit a brace-escape bug from this choice. The convention is known and named; the AI continues to produce the wrong-tool choice. This is one of 10+ entries in the cross-cutting note where codified guidance against the AI-typical shape coexists with continued violation.
