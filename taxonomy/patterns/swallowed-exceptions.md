...
The AI-amplified observation is *scale and lack of justification*. AI-generated codebases produce the pattern at densities that human authorship rarely reaches (1,066 sites in one project), apply it to operations that have no plausible best-effort justification (one-line dict access, library initialization), and reproduce it across hundreds of unrelated functions without architectural reason. The 278 instances in `pollypm/cockpit_ui.py` alone is a single UI file where the AI has wrapped essentially every event handler in a defensive `try/except: pass`.
...
**This is the named evergreen the project plan called for.** CLAUDE.md lists "swallowed exceptions" alongside "off-by-one" and "swapped args" as the canonical evergreens that the taxonomy should cover. The other two are still to come. Their landing should complete the project plan's named-evergreen set.
...
**The "linter silenced rather than code fixed" sub-shape** observed in pollypm is itself a small meta-observation worth tracking. When a codebase universally `noqa`s the rule that should prevent an anti-pattern, the convention has inverted — the rule's existence is now read as a nuisance rather than a constraint. This is a small but specific failure mode of AI-assisted development where the AI generates rule-violating code and is asked to "make the linter happy," which it does by adding the suppression rather than fixing the underlying code. Adjacent to but distinct from the inconsistent-error-handling pattern. Could be a future entry if more specimens accumulate.

**Adjacent patterns in evidence/patterns-draft/.** Several swallowed-exception sub-shapes captured in the marianne-ai-compose#308 specimen (predicate-returning-bare-False, validator-returning-False-on-error, fire-and-forget exception discard) are real but documented under this umbrella rather than as separate entries. If a future audit produces enough cross-context evidence on the predicate-returning-False shape specifically (where the function's name promises a truthy/falsy answer but the falsy case secretly means "broke"), that could be promoted to its own entry. For now, all sub-shapes are documented here.
