...
Three captured specimens, each from a different AI-coded Python project, each demonstrating a different sub-shape. Specimens live in `evidence/github-issues/`.

- **[MiniMax-AI/MiniMax-MCP#64](https://github.com/MiniMax-AI/MiniMax-MCP/issues/64)** — clustered `requests.get` in MCP server. Six instances of `requests.get(url)` in `server.py` (voice clone, audio playback, video generation, image download) without `timeout=`. MiniMax-MCP is the **official MiniMax MCP server** (1476+ stars, MiniMax-AI organization). MCP server hangs the AI agent's tool pipeline. Contributor "Xuner AI" suggests AI signature in recent commits. Specimen: [MiniMax-AI-MiniMax-MCP-64.md](../../evidence/github-issues/2026-05-16-MiniMax-AI-MiniMax-MCP-64.md).
- **[NorthlandPositronics/Cogtrix#972](https://github.com/NorthlandPositronics/Cogtrix/issues/972)** — `subprocess.run` variant with same-project-knows-right-pattern shape. Four `subprocess.run` calls in `tools/github_tools.py` lack `timeout=`. The same project's `tools/shell.py` *has* timeout protection and `tools/http_request.py` clamps HTTP timeouts to 1–120s. Bot-authored audit (`is_bot: true`). Project contributors named after Futurama characters ("Turanga Leela", "Ami Wong"). Issue body explicitly compares the correct-pattern sites to the drifting site. Specimen: [NorthlandPositronics-Cogtrix-972.md](../../evidence/github-issues/2026-05-16-NorthlandPositronics-Cogtrix-972.md).
- **[dagster-io/dagster#33747](https://github.com/dagster-io/dagster/issues/33747)** — `requests.*` calls in developer CLI with AST-scan reproducer. Three sites in `dagster-dg-cli` omit `timeout=`. Audit author distilled the pattern into a reproducible AST check — methodologically reusable. Reference to prior partial-fix #17831 (timeouts added to schedule/sensor paths but not propagated to developer-CLI). Specimen: [dagster-io-dagster-33747.md](../../evidence/github-issues/2026-05-16-dagster-io-dagster-33747.md).
...
**Category `reliability`** — new category. Previous entries have used `structure`, `testing`, `defensive-programming`, `error-handling`, `control-flow`, `documentation`, `library-usage`, `async`, `consistency`, `observability`, `configuration`, `language-pitfall`. The new category captures patterns about *whether the program can be expected to run reliably under realistic deployment conditions*. Future entries about missing retry logic, missing health-check endpoints, hardcoded URLs without redundancy, or missing rate-limit handling could fit here.
...
**MCP-server context as a recurring sub-shape.** Three of the project's entries now have MCP-server-specific defect surfaces: [`print-instead-of-logging`](print-instead-of-logging.md) (print() corrupts stdout transport), [`wrong-tool-for-job`](wrong-tool-for-job.md) (shell-embedded Python in MCP skills), and this entry (missing timeouts hang the agent tool pipeline). As MCP adoption expands, MCP-server-specific patterns may merit their own consolidated note.
