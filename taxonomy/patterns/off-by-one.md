...
- **A function that "happens to work" for the current test values.** The mantis specimen documented this shape: the trim algebra works for the current `frames_per_inference` constants but the boundaries do not line up with the intent. If a small change to the constants would expose a defect, the code has an off-by-one waiting to fire.
