...
- **[NousResearch/hermes-agent#?](https://github.com/NousResearch/hermes-agent)** — hermes-agent has CLAUDE.md and has contributed specimens to five other entries; mutable-default-arguments may be present at scale (not yet captured for this entry).
...
**Category `language-pitfall`** — new category. Previous entries have used `structure`, `testing`, `defensive-programming`, `error-handling`, `control-flow`, `documentation`, `library-usage`, `async`, `consistency`, `observability`, `configuration`. The new category captures Python-specific footguns the AI inherits from tutorial corpus despite warnings being present in the same corpus. Future entries about late-binding closures, `is` vs `==` value comparison, integer caching pitfalls, or `__init__` super-call omission could fit here.
